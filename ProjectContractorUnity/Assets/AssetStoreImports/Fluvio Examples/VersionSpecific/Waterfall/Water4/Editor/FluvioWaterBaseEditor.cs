...
[CustomEditor(typeof(FluvioWaterBase))]
public class FluvioWaterBaseEditor : Editor 
{    
    public GameObject oceanBase;
    private FluvioWaterBase waterBase;
    ...
    {		
    	...
    	waterBase = (FluvioWaterBase)serObj.targetObject;
    	oceanBase = ((FluvioWaterBase)serObj.targetObject).gameObject;
    	if(!oceanBase) 
      		return;
		...
		if (!oceanMaterial) {
			sharedMaterial.objectReferenceValue = (Object)FluvioWaterEditorUtility.LocateValidWaterMaterial(oceanBase.transform);		
		...
		if(waterQuality.intValue > (int)FluvioWaterQuality.Low && !SystemInfo.supportsRenderTextures)
		...
		FluvioWaterEditorUtility.SetMaterialColor("_BaseColor", EditorGUILayout.ColorField("Refraction", FluvioWaterEditorUtility.GetMaterialColor("_BaseColor", oceanMaterial)), oceanMaterial);
		FluvioWaterEditorUtility.SetMaterialColor("_ReflectionColor", EditorGUILayout.ColorField("Reflection", FluvioWaterEditorUtility.GetMaterialColor("_ReflectionColor", oceanMaterial)), oceanMaterial);		
		...
		FluvioWaterEditorUtility.SetMaterialTexture("_BumpMap",(Texture)EditorGUILayout.ObjectField("Normals", FluvioWaterEditorUtility.GetMaterialTexture("_BumpMap", waterBase.sharedMaterial), typeof(Texture), false), waterBase.sharedMaterial);  
		if (hasShore)
			FluvioWaterEditorUtility.SetMaterialTexture("_ShoreTex", (Texture)EditorGUILayout.ObjectField("Shore & Foam", FluvioWaterEditorUtility.GetMaterialTexture("_ShoreTex", waterBase.sharedMaterial), typeof(Texture), false), waterBase.sharedMaterial);  
		...
		animationTiling = FluvioWaterEditorUtility.GetMaterialVector("_BumpTiling", oceanMaterial);
		animationDirection = FluvioWaterEditorUtility.GetMaterialVector("_BumpDirection", oceanMaterial);
		...
		FluvioWaterEditorUtility.SetMaterialVector("_BumpTiling", animationTiling, oceanMaterial);
		FluvioWaterEditorUtility.SetMaterialVector("_BumpDirection", animationDirection, oceanMaterial);    				

		Vector4 displacementParameter = FluvioWaterEditorUtility.GetMaterialVector("_DistortParams", oceanMaterial);
		Vector4 fade = FluvioWaterEditorUtility.GetMaterialVector("_InvFadeParemeter", oceanMaterial);
		...
		float gerstnerNormalIntensity = FluvioWaterEditorUtility.GetMaterialFloat("_GerstnerIntensity", oceanMaterial);
		gerstnerNormalIntensity = EditorGUILayout.Slider("Per Vertex", gerstnerNormalIntensity, -2.5F, 2.5F);
		FluvioWaterEditorUtility.SetMaterialFloat("_GerstnerIntensity", gerstnerNormalIntensity, oceanMaterial);
		...
		if(!oceanMaterial.HasProperty("_Fresnel")) {
			if(oceanMaterial.HasProperty("_FresnelScale")) {
				float fresnelScale = EditorGUILayout.Slider("Intensity", FluvioWaterEditorUtility.GetMaterialFloat("_FresnelScale", oceanMaterial), 0.1F, 4.0F);
				FluvioWaterEditorUtility.SetMaterialFloat("_FresnelScale", fresnelScale, oceanMaterial);
			}			
		...
		else
		{
			Texture fresnelTex = (Texture)EditorGUILayout.ObjectField(
					"Ramp", 
					(Texture)FluvioWaterEditorUtility.GetMaterialTexture("_Fresnel", 
					oceanMaterial), 
					typeof(Texture),
					false);
			FluvioWaterEditorUtility.SetMaterialTexture("_Fresnel", fresnelTex, oceanMaterial);
		}
		
		EditorGUILayout.Separator ();
		    			
		FluvioWaterEditorUtility.SetMaterialVector("_DistortParams", displacementParameter, oceanMaterial);
		...
		if (edgeBlend.boolValue)
		{
			...
			FluvioWaterEditorUtility.SetMaterialVector("_InvFadeParemeter", fade, oceanMaterial);
		}
		EditorGUILayout.Separator ();					
		...
		if(oceanMaterial.HasProperty("_Foam")) {
			...
			Vector4 foam = FluvioWaterEditorUtility.GetMaterialVector("_Foam", oceanMaterial);
			
			foam.x = EditorGUILayout.Slider("Intensity", foam.x, 0.0F, 1.0F);
			foam.y = EditorGUILayout.Slider("Cutoff", foam.y, 0.0F, 1.0F);
			
			FluvioWaterEditorUtility.SetMaterialVector("_Foam", foam, oceanMaterial);			
		}
